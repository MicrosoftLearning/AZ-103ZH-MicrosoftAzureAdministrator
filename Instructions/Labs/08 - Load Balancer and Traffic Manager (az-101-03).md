...
1. 在 **“编辑模板”** 边栏选项卡中，加载模板文件 **Labfiles\\Module_08\\Load_Balancer_and_Traffic_Manager\\az-101-03_01_azuredeploy.json**。 
   ...
   > **注意**：查看模板的内容，注意其定义了将托管 Windows Server 2016 Datacenter Core 的两个 Azure VM 部署到可用性集中。
...
1. 在 **“编辑模板”** 边栏选项卡中，加载模板文件 **Labfiles\\Module_08\\Load_Balancer_and_Traffic_Manager\\az-101-03_01_azuredeploy.json**。 
   ...
   > **注意**：这是你在上一任务中使用的同一模板。您将使用它将一对 Azure VM 部署到第二个区域。 
...
#### 任务 1：在第一个区域中实施 Azure 负载均衡规则
   ...
   > **注意**：开始此任务之前，请确保您在上次练习的第一个任务中启动的模板部署已经完成。 
...
1. 从 **az1010301w-lb** 边栏选项卡，显示 **az1010301w-lb** 后端池边栏选项卡。

1. 从 **az1010301w-lb** -后端池边栏选项卡，添加具有以下设置的后端池：
...
1. 从 **az1010302w-lb - 后端池** 边栏选项卡，添加具有以下设置的后端池：
   ...
   > **注意**：可能会按相反顺序分配 Azure VM 的 IP 地址。 
...
1. 从 **az1010301w-lb** 边栏选项卡，显示 **az101031w-lb-入站 NAT** 规则边栏选项卡。

   > **注**：NAT 功能不依赖于运行状况探测。

1. 从 **az1010301w-lb-入站 NAT** 规则边栏选项卡，添加具有以下设置的第一个入站 NAT 规则：
    ...
    - 网络 IP 配置：**ipconfig1 (10.101.31.4)** 或 i**pconfig1 (10.101.31.5)**
    ...
    > **注意**：等待操作完成。这应该需要不到一分钟。

1. 从 a**z1010301w-lb - 入站 NAT 规则** 边栏选项卡，添加具有以下设置的第二个入站 NAT 规则：
...
1. 从 **az1010302w-lb - 入站 NAT 规则** 边栏选项卡，添加具有以下设置的第一个入站 NAT 规则：
    ...
    - 网络IP配置：**ipconfig1 (10.101.32.4)** 或 **ipconfig1 (10.101.32.5)**
...
1. 在逻辑阵列模块虚拟机中，使用您在上一任务中确定的 IP 地址替换&lt;az1010301w-lb_public_IP&lt;占位符后，运行以下命令：
   ...
   > **注意**：此命令使用您在上一任务中创建的 **az1010301w-vm0-RDP** NAT 规则，启动到 **az1010301w-vm0** Azure VM 的远程桌面会话。
...
> **结果**：完成本练习后，你已在两个区域中实现并验证了 Azure 负载均衡器的负载均衡规则和 NAT 规则。


### 练习 2：实现 Azure 流量管理器负载均衡
...
#### 任务 1：将 DNS 名称分配给 Azure 负载均衡器的公共 IP 地址
   ...
   > **注意**：此任务是必要的，因为每个流量管理器终结点都必须分配一个 DNS 名称。 
...
1. 使用搜索结果列表导航到 **创建流量管理器个** 人资料边栏选项卡。
...
1. 在“流量管理器配置文件”边栏选项卡中，显示其 **“配置”** 边栏选项卡并查看配置设置。
   ...
   > **注意**：流量管理器配置文件 DNS 记录的默认 TTL 为 60 秒
...
1. 从 Azure 门户，在 Cloud Shell 窗格中启动 PowerShell 会话。 
   ...
   > **注意**：  如果这是您第一次在当前 Azure 订阅中启动 Cloud Shell，则会要求您创建 Azure 文件共享以保留 Cloud Shell 文件。如果是，接受默认设置，这样会在自动生成的资源组中创建存储帐户。
